...
from app.infrastructure.repositories import PostgresTaskRepository, PostgresReminderRepository
...
async def get_current_user(initData: str = Header(default="", alias="initData")) -> int:
    ...
    return user_data["id"]  # Возвращаем tg_id
...
async def get_task_service(session: AsyncSession = Depends(get_db_session)) -> TaskService:
    ...
    return TaskService(task_repo, reminder_repo)